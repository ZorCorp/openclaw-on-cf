...
Deploy your own **OpenClaw** bot — an AI agent with both a **Telegram** channel
and a **web dashboard** — to **your own Cloudflare account**.

OpenClaw runs as a Cloudflare **Worker + Container**, persists state to **R2**,
and talks to **Workers AI** through a shared **AI Gateway**. Telegram runs in
**webhook** mode; the dashboard logs in with a Gateway Token.
...
│   └── moltworker/     ← the OpenClaw Worker + Container + dashboard source
├── .gitignore
└── README.md
```
